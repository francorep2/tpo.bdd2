...
public class BookingRestController {
    ...
    @PutMapping("/{bookId}")
    public BookingDTO updateBooking(@PathVariable UUID bookId, @RequestBody BookingDTO bookingDTO) {
        return bookingService.updateBooking(bookId, bookingDTO);
    }
    
    @DeleteMapping("/{bookId}")
    public ResponseEntity<Void> deleteBooking(@PathVariable UUID bookId) {
        bookingService.deleteBooking(bookId);
        return ResponseEntity.noContent().build(); 
    }
    
    @GetMapping("/{bookId}")
    public Booking getBookingById(@PathVariable UUID bookId) {
    ...
    public List<Booking> getBookingsByClientId(@PathVariable UUID clientId) {
